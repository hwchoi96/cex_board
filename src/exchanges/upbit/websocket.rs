use crate::exchanges::upbit::UpbitWsTicker;
...
/// 한 번 연결해 끊길 때까지 읽기 (재연결 없음). 상위에서 [`run_quote_websocket_with_reconnect`] 권장.
async fn connect_quote_websocket(
    pairs: Vec<String>,
    sender: Sender<UpbitWsTicker>,
...
pub async fn run_quote_websocket_with_reconnect(
    pairs: Vec<String>,
    sender: Sender<UpbitWsTicker>,
) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    loop {
        ...
        let result = connect_quote_websocket(pairs.clone(), sender.clone()).await;
...
async fn try_send_ticker(tx: &mpsc::Sender<UpbitWsTicker>, json: &str) {
